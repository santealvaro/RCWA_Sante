...
def TransRefl(N, e_m, e_d, f1, f2, period, d, e, wavelength, theta, Num_ord, permittivity_matrix):
    ...
    for a multilayer grating using TE polarization.
    ...
    if permittivity_matrix is None:
        ...
        for ind in range(N):
            ...
            Q_i, V_i, W_i = TE_matrixes.matrixesQVW_RectangularGrating(
        ...
        #In case convolution matrixes created with FFT want to be used
        ind = 0
        # for row in permittivity_matrix:            
        #     Q_i, V_i, W_i = Matrix_Gen_TE.matrixesQVW_AnyGrating(
        #         Num_ord, row, period, e[0], theta, wavelength
        #     )
    ...
    Y_inc = np.diag(Kz1 / k0)
    Y_sub = np.diag(Kz3 / k0)
    ...
    for ind in range(N-1, -1, -1):
        ...
        f_i = W[ind] @ (I + X[ind] @ a_i @ inv_b @ X[ind])
    ...
    T1 = np.linalg.inv(1j * Y_inc @ f[0] + g[0]) @ (
        1j * Y_inc @ Dirac_del + 1j * np.sqrt(e[0]) * np.cos(theta) * Dirac_del
    )
    R = f[0] @ T1 - Dirac_del
    ...
    DEt = (T * np.conj(T)) * np.real(Kz3) / (k0 * np.sqrt(e[0]) * np.cos(theta))
    
    Ref = np.sum(DEr)
    Tran = np.sum(DEt)

    return np.real(Ref), np.real(Tran) #Ref and Tran are real, but the have a very little imaginary part (e^-18 or so)
